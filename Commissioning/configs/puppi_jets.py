...
process.load("flashgg/MicroAODProducers/flashggPUPPIJets_cfi")

#Tag stuff
process.load("flashgg/TagProducers/flashggDiPhotonMVA_cfi")
process.load("flashgg/TagProducers/flashggPUPPIVBFMVA_cfi")
process.load("flashgg/TagProducers/flashggVBFDiPhoDiJetMVA_cfi")
process.load("flashgg/TagProducers/flashggPUPPITags_cfi")
...
process.flashggTreeMakerWithTagSorter = cms.EDAnalyzer('FlashggFlashggTreeMakerWithTagSorter',
		...
		VertexCandidateMapTagDz=cms.InputTag('flashggVertexMapUnique'),
		VertexCandidateMapTagAOD = cms.InputTag('flashggVertexMapValidator'),
		JetTagDz = cms.InputTag("flashggPUPPIJets"),
		DiPhotonTag = cms.untracked.InputTag('flashggDiPhotons'),
		METTag = cms.untracked.InputTag('slimmedMETs'),
		PileUpTag = cms.untracked.InputTag('addPileupInfo'),
...
process.TFileService = cms.Service("TFileService",
		fileName = cms.string("puppi_tree.root")
		)
...
process.p = cms.Path(process.flashggVertexMapUnique*
		...
		process.puppi*
		process.ak4PFJets*
    (process.flashggDiPhotonMVA+process.flashggPUPPIJets)* # These two could run in parallel, so use +
		process.flashggUntaggedCategory*
		(process.flashggPUPPIVBFMVA)* # Needs to happen after Jets
